...
def GenCaseList(configName, caseVar, caseValList, trimList, trimListConst):

    caseList = []
    for iCase, caseVal in enumerate(caseValList):
        for iTrim, trimSet in enumerate(trimList):
            caseName = "%s_%s%d" %(configName, caseVar, caseVal)
            case = {
                    'configName': configName,
                    'name': "%s_%s%d" %(configName, caseVar, caseVal),
...
#%%
def ExecuteCase(avlPath, case):

    # Determine OS type to determine which AVL executable to use
    sysType = platform.system()
    if sysType is 'Linux':
        avlExec = 'avl'
    elif sysType is 'Windows':
        avlExec = 'avl.exe'
    elif sysType is 'Darwin': #Mac
        avlExec = 'avl3.35'

    # Join Path and Exec
    avlRun = os.path.join(avlPath, avlExec)

    # Full path
    avlPath = os.path.abspath(avlPath)

    # Out and Script Paths
    outPath = os.path.join(avlPath, case['configName'], 'out')
    scriptPath = os.path.join(avlPath, case['configName'], 'script')

    # Temporary path to place files, then move them to outPath (This gets around a directory length limit in AVL)
